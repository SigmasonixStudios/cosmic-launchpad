...
impl<'a, Message: Clone + 'static> ApplicationButton<'a, Message> {
    ...
    pub fn new(
        ...
        let source_icon = match source {
            Some((_, source_icon_handle)) => source_icon_handle.as_ref().map(|i| {
                Element::from(
        ...
        let label_height = if show_label { 44.0 } else { 0.0 };
        let tile_width = icon_size + if show_label { 64.0 } else { 20.0 };
        let tile_height = icon_size + if show_label { 52.0 } else { 20.0 };
        let label: Element<'_, Message> = if show_label {
            container(
                text(name)
                    .size(12.0)
                    .width(Length::Fill)
